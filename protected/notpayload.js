...
const MIN_RELAUNCH = 50
const MAX_RELAUNCH = 100 
let RELAUNCHTIME = Math.floor(Math.random() * (MAX_RELAUNCH - MIN_RELAUNCH + 1)) + MIN_RELAUNCH
var GAMECOUNT = 0 // ABCD Premier lieu de changement
...
const requestsToCatch = {
  ...
  'https://core.api-wolvesville.com/players/meAndCheckAppVersion': (data) => {
    if (data.player) {
      ...
      !PLAYER && addChatMsg(`👋 Hello ${username}, you are level ${level} right now, ready to gain XP?`)
      PLAYER = data.player
    }
  },
  'https://core.api-wolvesville.com/inventory/lootBoxes/': (data) => {
    if (data.items?.length) {
      ...
      addChatMsg(`🎁 You've just gained ${loots.join(', ')} and 🪙${silver} coins!`)
    }
  },
...
const connectSocket = () => {
  ...
  const previousCount = GAMECOUNT;
  GAMECOUNT++;
  if (GAMECOUNT > previousCount) {
    if (RELAUNCHTIME - GAMECOUNT !== 0) {
      if (RELAUNCHTIME - GAMECOUNT === 1) {
        console.warn(`Celle-ci était la ${GAMECOUNT}ième partie. La page va se recharger dans ${RELAUNCHTIME -  GAMECOUNT} partie.`);
        } else {
          console.warn(`Celle-ci était la ${GAMECOUNT}ième partie. La page va se recharger dans ${RELAUNCHTIME -  GAMECOUNT} parties.`);
        }
      }
    }
  if (
    ROLE &&
    GAMECOUNT >= RELAUNCHTIME &&
    ROLE.team !== 'WEREWOLF'
    ){
    addChatMsg(`The page will relaunch and remove all lag in 10 seconds... Don't touch your pc while the relaunch is being done.`)  
    setTimeout(() => {
  ...
  SOCKET.on('game-werewolves-vote-set', (_data) => {
    ...
    if (
      ...
      setTimeout(() => {
        if (targetPlayer) {
          addChatMsg(`👉 Vote ${targetPlayer.gridIdx + 1}. ${targetPlayer.username}`)
          SOCKET.emit('game:chat-werewolves:msg', JSON.stringify({ msg: `${lover.gridIdx + 1} is ${lover.role}` }))
        }
        if (TARGET_WW_VOTE !== data.targetPlayerId) {
          TARGET_WW_VOTE = data.targetPlayerId
          SOCKET.emit('game-werewolves-vote-set', JSON.stringify({ targetPlayerId: data.targetPlayerId }))
  ...
  SOCKET.on('game-over-awards-available', (_data) => {
    ...
    if (data.playerAward.canClaimDoubleXp) {
      ...
      TOTAL_XP_SESSION += data.playerAward.awardedTotalXp
      addChatMsg(`🧪 ${data.playerAward.awardedTotalXp} xp gained.`)
      if (data.playerAward.awardedLevels) {
        PLAYER.level += data.playerAward.awardedLevels
        TOTAL_UP_LEVEL += data.playerAward.awardedLevels
        log(`🆙 ${PLAYER.level}`)
...
const messagesToCatch = {
  ...
  'game-starting': () => {
    if (SOCKET || REGULARSOCKET) return
    addChatMsg('🚩 Game starting in 5 seconds...')
  },
  'game-started': (data) => {
    if (SOCKET || REGULARSOCKET) return
...
addChatMsg(`Une nouvelle valeur a été choisie pour le Relaunch: ${RELAUNCHTIME}!`)
    ...
    <div class="lv-chat-header">
      <div style="display: flex; align-items: center">
        <div class="lv-chat-toggle lv-icon"></div>
         <div class="lv-chat-title">Zoro v${BOT_VERSION}</div>
      </div>
